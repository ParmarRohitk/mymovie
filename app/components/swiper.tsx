...
const Swiper: React.FC<MovieScrollProps> = ({ title, movies, slug }) => {
    ...
    return (
        <div className="relative p-4 bg-gradient-to-b from-gray-500 to-gray-700 rounded-lg">
            ...
            >
                {movies.map((movie) => (
                    <Link href={`/${movie.category}/${movie.slug}`} key={movie.id} className="p-2">
                        <div className="min-w-[200px] w-[200px] bg-gradient-to-b from-gray-400 to-gray-300 p-2 rounded-lg shadow-lg transition-transform transform hover:scale-105">
                            <img
                                src={movie.image}
                                alt={movie.name}
